...
export default function ReceiveShipment() {
  ...
  async function generate() {
    ...
    for (let i = 0; i < n; i++) {
      ...
      while (existingIds.has(id)) { seq++; id = makeToteId(seq + i, today); }
      existingIds.add(id);

      const tote: Tote = {
        id, productId, status: 'in_yard', location: { kind: 'yard' },
        currentQtyGal: TOTE_CAPACITY_GAL, receivedAt: nowIso,
        createdBy: 'jacob', syncState: 'synced',
        updatedAt: nowIso, updatedBy: 'jacob', updatedLabel: 'Received',
      };
      await putTote(tote);
      await appendEvent({
        id: uuid(), toteId: id, type: 'received', createdAt: nowIso,
        createdBy: 'jacob', payload: { productId, qty: TOTE_CAPACITY_GAL }, synced: true,
      });
      created.push(id);
    }
    ...
    <Layout title="Receive Shipment" back="/">
      <div className="space-y-3">
        <div className="card p-3 space-y-3">
          ...
          <div>
            <label className="label block mb-1">Number of Totes</label>
            <input
            ...
            <p className="text-xs text-ink-muted mt-1">
              Each tote: {TOTE_CAPACITY_GAL} gal.
            </p>
          </div>
        </div>
        <button className="btn-primary w-full" disabled={saving} onClick={generate}>
        ...
        {generated.length > 0 && (
          <div className="card p-3">
            ...
            <div className="flex flex-wrap gap-1">
              {generated.map((id) => (
                <span key={id} className="text-[11px] font-mono px-1.5 py-0.5 rounded bg-emerald-50 border border-emerald-200 text-emerald-800">
                  {id}
                </span>
              ))}
            </div>
